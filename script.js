...
var pomodoro = {workTime:25, breakTime:5, remaining:25*60, isRunning:false, isBreak:false, timerId:null, freeMode:false};
...
function attachAppEvents(){
  ...
  document.querySelectorAll('[data-save-exam]').forEach(function(el){el.onclick=function(){var title=document.getElementById('exam-title').value;var date=document.getElementById('exam-date').value;var time=document.getElementById('exam-time').value;var subjectId=document.getElementById('exam-subject').value;var room=document.getElementById('exam-room').value.trim();if(!title||!date){alert('Titre et date requis');return}state.exams.push({id:generateId(),title:title,date:date,time:time,subject_id:subjectId||null,room:room});state.modal=null;saveState();render()}});
